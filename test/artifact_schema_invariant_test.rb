...
class ArtifactSchemaInvariantTest < Minitest::Test
  def test_committed_capture_artifacts_validate_against_schema_and_invariants
    manifest_path = File.join(repo_root, "site", "data", "captures", "manifest.json")
    manifest = JSON.parse(File.read(manifest_path))
    datasets = manifest.fetch("combos").to_h do |entry|
      dataset_path = File.join(repo_root, "site", "data", "captures", entry.fetch("file"))
      [entry.fetch("id"), JSON.parse(File.read(dataset_path))]
    end

    pipeline = ThriftIllustrated::ValidationPipeline.new(schema_dir: File.join(repo_root, "site", "data", "schemas"))
